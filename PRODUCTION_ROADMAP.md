...
> **작성 2026-09-09 · 축약 2026-09-16 · F-02 완료 2026-09-17.** 50건 중 **45건이 끝났다.**
...
생성기 · 편집 안전장치 · 검수 도구 · **오써링 GUI(F-02)** 까지 왔다. 남은 5건은 성격이 셋으로 갈린다.

| 남은 것 | 왜 안 끝났나 |
|---|---|
| **G-01** 미측정 실행 · **G-02** 소크·카오스 · **G-05** 수용 기준 v2 | 코드가 아니라 **시간·장비·비용**이 필요하다. 72시간 연속 회차 · 전량 프리베이크 · 블라인드 평가 참가자. **P0 다** — 이것 없이는 상용 판정을 할 수 없다 |
| **F-07** LLM 보조 오써링 | XL. 밑에 깔릴 것(E-02 스키마 · E-03 MCP · F-02 Studio · F-03 설명 · F-04 안전장치)은 다 끝났다 |
...
- [x] **F-02** NPC Studio (웹): 직업·NPC 카드 · 하루 예측 재생 · 폼 편집 · 하루 일과 편집기 · 버킷 그리드 · 새 직업 마법사(한 트랜잭션) · 연습장 · 되돌리기 · 인라인 검증 — P1 · XL · 의존 E-02, F-03, F-04 — **남음: LLM 제안 리뷰(F-07) · git 커밋(계획 밖)**
...
---
### F-02 NPC Studio (웹) — 완료 (2026-09-17)
...
**테스트.** 화면 렌더는 단언하지 않는다(CLAUDE.md §5.1 · 1차 T20 결정). 서비스 층의 불변식만 본다 — 무변경 저장 바이트 동일 · 검증 실패 시 원본 불변 · 번호 재배치 거절 · 외부 편집 거절 · 트랜잭션 복원 · 마법사가 만질 파일 목록 드리프트 · 화면 용어 드리프트. 화면은 판정 시나리오를 손으로 밟는다.

**완료 조건.** 콘텐츠 디자이너가 튜토리얼 **5·7장을 JSON 을 열지 않고** Studio 만으로 끝낸다
(9장 액션 추가는 프롬프트 프리픽스를 흔들어 미리 구운 계획이 전량 무효가 되므로 **읽기 전용**이다 —
1차 계획 §1.2 가 "액션 편집은 밖" 이라 정했고 여기가 그것을 따른다).
LLM 제안 리뷰와 git 커밋은 **F-07 로 넘긴다**.

**크기·의존.** XL. 의존 E-02, F-03, F-04. — **완료. 보강은 `STUDIO_IMPROVEMENT_PLAN.md`(34건)와
`STUDIO_IMPROVEMENT_PLAN_2.md`(28건)가 기록이다.**
...
| 콘텐츠 | 아키타입 추가에 코드 변경 0 · Studio 로 5·7장 완주 + 9장(액션 추가)은 읽기로 확인 · pinned ≥ 10 | 통과 | F-05, F-02, F-06 |
...
남은 5건이 만들 것만 적는다. 완료분이 만든 것은 `CODEMAP.md` 가 안내한다.
