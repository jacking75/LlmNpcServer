...
| 검수 도구 | **구현됨** | `npc review` — 층화 추출(폴백 대체분 포함) · F-03 설명 · 다양성 대조 3건 · 반려 이력 · `[e] 수정` 이 4단 재검증 후 `pinned/` 저장 · 폐기 사유 코드 | F-06 |
...
**구현 결과.** `tests/agent-bench/`(과제 10종 + 채점 공통 함수) · `tools/agent-bench.ps1`(채점·기록·자체 시험) · `docs/measurements/agent_bench.csv` · `reference_metrics.html` §13.5.

- **러너가 에이전트를 부르지 않는다.** 에이전트마다 호출 방식이 다르고, 러너가 그것을 흉내 내면 **"이 러너로 잰 점수"** 가 되어 버린다. 사람이 `task.md` 를 그 에이전트의 제 방식대로 주고, 러너는 채점과 기록만 한다.
- **`-SelfTest` 가 채점기를 시험한다.** 과제를 풀지 않은 상태에서 10종을 전부 채점해 하나라도 통과하면 실패로 끝난다 — **언제나 통과하는 채점기는 점수가 아니라 상수다.** 첫 실행에서 10종 전부 실패(정상)했다.
- **과제 7 의 표본은 실제 반려 플랜이다.** `planstore/rejected/`(766건)에서 서로 다른 실패 코드 8종을 뽑았고, 8건 전부 지금도 검증에 떨어지는 것을 확인했다. 합성 표본을 쓰면 "이 벤치에서만 나는 실패" 를 재게 된다.
- **거절이 정답인 과제를 넣었다**(09). 이것이 없으면 벤치는 "시키는 대로 하는가" 만 잰다.
- 시각을 인자로 받는다 — 스크립트가 벽시계를 읽으면 같은 회차를 다시 채점했을 때 다른 줄이 남는다.

**실측 — 1건뿐이다.** `04-fallback-plan` 을 이 세션(`claude-opus-5`)이 풀어 4/4 통과했다. **자기가 낸 문제를 자기가 푼 점수는 능력의 측정이 아니라 채점기가 도는지의 확인이다** — 성공률로 읽지 않는다. 10종 × 3회와 팩 전후 대조는 **미실시**로 `reference_metrics.html` §14 에 올렸다: 깨끗한 트리에서 에이전트 세션 30회가 필요하고, 그것은 사람이 시키는 일이다.

**테스트.** `tests/Npc.Tests/Docs/AgentBenchTests.cs`(8). 과제·채점기 짝, front matter, 파일 온전성(BOM·괄호), 공통 함수 사용, 표본 8건, 거절 과제 존재, 러너의 벽시계 금지, CSV 머리글 일치를 본다.
...
**구현 결과.** `npc review` — `tools/Npc.Cli/Review/`(`ReviewSampler`·`ReviewRecord`·`ReviewCommand`). `tools/review.ps1` 은 지웠다.

- **v1 이 못 하던 넷을 고쳤다.** (1) 판정에 필요한 것을 한 화면에 — F-03 플랜 설명(트레이스·수지·시간) · 아키타입 성향·목표 · **같은 아키타입의 다른 버킷 3개 시퀀스**(다양성 대조) · 같은 버킷의 반려 이력. (2) `[e] 수정` 이 실제로 고친 플랜을 받아 **4단 재검증 후** `pinned/` 에 `origin: Pinned` 로 저장한다. (3) 폴백 대체분이 표본에 든다. (4) 폐기 사유가 코드(`Situation`·`Character`·`Route`·`Survival`)라 집계된다 — `system_rules.md` 의 "WHAT MAKES A PLAN GOOD" 4항목 그대로다.
- **"폴백 대체분" 을 "생성을 시도했는데 떨어진 버킷" 으로 정의했다.** 한 번도 시도하지 않은 버킷(프리베이크 대상 밖 2,162개)까지 넣으면 모집단의 4분의 3이 폴백이 되고, 검수자는 같은 손글씨 플랜 40개를 계속 보게 된다 — 그것은 검수가 아니다. 판정 기준은 `rejected/` 에 그 버킷의 반려 기록이 있는가다.
- **판정 시간을 게이트에서 뺐다.** v1 은 절감률 게이트가 시간을 봤는데, 그러면 **꼼꼼히 볼수록 성적이 나빠진다**. 기록은 계속 하되 보고용이다.
- **비대화 판정 경로**(`--bucket … --verdict … --plan …`)를 뒀다. 사람의 판정을 대신하지 않는다 — 판정은 인자로 들어오고 이 경로가 하는 일은 저장과 기록뿐이다. 검수 회차를 스크립트로 나눠 돌릴 때 필요하다.
- 겸사겸사 아키타입 폴백을 스토어에 먼저 올린다. 안 올리면 폴백 대체분이 전부 최후 플랜(`Wait>Emote>Rest`)으로 보여 검수자가 진짜 폴백을 못 본다.

**완료 조건 — 부분 달성.** `planstore/pinned/` 에 **19건**이 생겼고 `manifest.counts.pinned = 19` 다. 호스트가 실제로 그것을 올린다(`버킷 737/2880 (pinned 19)`). **단 이 19건은 사람의 심미 판정이 아니다** — C-05 자동 수선이 고치고 4단 재검증을 통과한 것을 `--verdict edit` 로 저장한 것이고, 기록의 `note` 에 그렇게 적혀 있다. **사람이 40건을 보는 회차는 미실시**다: 그것은 사람이 하는 일이고, 여기서 대신하면 §13 "pin 유지" 의 근거가 거짓이 된다.

**테스트.** `tests/Npc.Tests/Cli/ReviewTests.cs`(10). 층화 추출 결정론(같은 시드=같은 표본·다른 시드=다른 표본) · 폴백 대체분 포함 · 층 비율 ±1 · 기록 형식의 옛 필드 호환 · 빈 필드 생략 · 이스케이프 · `pinned/` ≥ 10건이 `PlanOrigin.Pinned` 로 로드 · manifest 수 일치.
...
**구현 결과.** `npc perf --check` — `tools/Npc.Cli/PerfCommand.cs`. 입력은 `docs/measurements/W10_load.csv`, 기준선은 `docs/measurements/perf_baseline.csv`(**커밋한다**).

| 규칙 | 판정 |
|---|---|
| `bytes_per_tick` | 0 이 아니면 실패. **계기는 이것 하나다** — `gen0` 은 프로세스 전역이라 대시보드 HTTP·소켓이 전부 든다 (§4) |
| `tick_p99` | 20ms 초과면 실패 |
| `scan_per_tick` | 150 초과면 실패. **`scan_cap == 0` 인 대조 회차는 뺀다** — 상한을 일부러 끄고 돌린 셀을 위반으로 세면 그 실험이 영영 빨간불이고, 사람이 판정을 꺼 버린다 |
| `tick_overruns` · `sequence_gaps` | 0 이 아니면 실패 |
| `p99_regression` | 기준선 × **1.3** 초과면 실패. 1.0 으로 두지 않는다 — 같은 코드도 회차마다 흔들리고, 그때마다 빨간불이면 사람이 이 판정을 끈다. 기준선에 없는 새 셀은 회귀로 세지 않는다(그러면 매트릭스를 넓힐 수 없다) |

- **워크플로 파일이 아니라 판정 명령이다.** 판정을 CI 설정에 적어 두면 로컬에서 같은 답을 얻을 수 없고, 그러면 "CI 에서만 빨간불" 이 된다 — 그 상태에서는 고치기 전에 원인을 모른다 (A-09 가 CI 워크플로를 제외한 것과 같은 논거).
- **기준선 갱신은 사람이 명시한다** (`--write-baseline --apply`). 도구가 조용히 기준을 낮추면 회귀가 회귀로 보이지 않는다.

**완료 조건 확인 — 실제로 빨간불이다.** 지금 커밋된 부하 결과(14셀)로 `npc perf --check` 는 `exit 0`(p99 최악 2.911ms · 할당 0). 첫 셀에 **틱당 할당 4,096B 와 p99 24.5ms**(= 틱 루프에 LINQ 한 줄을 넣었을 때 나오는 모양)를 넣은 CSV 로 돌리면 `exit 1` 이고 `bytes_per_tick` · `tick_p99` · `p99_regression` 세 규칙이 동시에 잡힌다.

**테스트.** `tests/Npc.Tests/Cli/PerfCheckTests.cs`(12). 절대 규칙 4종이 각각 발화 · 할당만으로도 실패 · 대조 회차 면제 · 회귀 폭 경계(1.3 위아래) · 새 셀 면제 · 기준선이 커밋돼 있음 · **커밋된 부하 결과가 자기 기준선을 통과함** · 기준선 왕복.
