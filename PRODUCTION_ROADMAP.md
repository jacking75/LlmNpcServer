...
- [ ] **A-01** NPC 상태 스냅샷·복구 (영속성) — P0 · L · 의존 없음
- [ ] **A-02** SIGTERM·정상 종료·`Bye(Shutdown)`·서비스 프로파일 — P0 · S · 의존 A-01
- [x] **A-03** 헬스체크(`/healthz/*`)·`LinkState` 노출·`Faulted` 좀비 제거 — P0 · S · 의존 없음
- [ ] **A-04** 설정 소스 통합 (CLI → 환경변수 → 파일) · 바인드 주소 · 프로파일 — P0 · M · 의존 없음
- [ ] **A-05** 관측성: OpenTelemetry 익스포터 · Prometheus 엔드포인트 · 구조화 로깅 · 알람 싱크 — P0 · M · 의존 A-04
- [ ] **A-06** 링크 보안: 핸드셰이크 인증(HMAC+nonce) · TLS/mTLS · 관리 API 인증 — P0 · M · 의존 B-01
- [ ] **A-07** 핫 리로드: 플랜 스토어 · 인터럽트 · 폴백 · few-shot (구조 변경은 재기동) — P1 · L · 의존 A-01, B-04
- [ ] **A-08** 전역 `NpcId` · 샤드 식별 · 다중 링크 세션 (샤딩 1단계) — P1 · L · 의존 B-01, B-05
- [ ] **A-09** 배포: Dockerfile · compose(테스트베드 포함) · CI 파이프라인 · 중앙 패키지 관리 · 릴리스 버저닝 — P0 · M · 의존 없음
- [ ] **A-10** 게임 시각 복원(`StartTick`) · `TickSync` 워치독 — P0 · S · 의존 A-01
- [ ] **A-11** 킬스위치 가역화 · `/admin/*` 네임스페이스 · 감사 로그 — P1 · S · 의존 A-06

### 트랙 B — 게임서버 연동 계약 (남의 서버에 붙는다)

- [ ] **B-01** 계약 버전(`ContractVersion`) · 와이어 버전 협상 · 기능 비트 — P0 · M · 의존 없음
...
- [ ] **B-04** 핸드셰이크 해시 분할 (구조 해시 / 내용 해시) · 부분 호환 정책 — P1 · S · 의존 B-01
...
- [ ] **B-07** 게임서버 적합성 테스트 키트 (`Npc.Conformance`) — P1 · M · 의존 B-01
- [ ] **B-08** 읽기 전용 질의 API (벌크·검색·스트림) — 링크가 아니라 HTTP — P1 · M · 의존 A-06

### 트랙 C — LLM 운영 (제공사가 죽어도, 예산이 바닥나도, 프롬프트를 바꿔도)

- [ ] **C-01** 제공사 페일오버 체인 · 런타임 재시도/백오프 · 엔진별 브레이커 — P0 · M · 의존 없음
- [ ] **C-02** 예산 알람 채널 · 임계 경보(80/95/100%) · 청구 주기 정합 — P0 · S · 의존 A-05
- [ ] **C-03** 프롬프트 버저닝·프리픽스 아티팩트 보관·플랜 스토어 롤백 — P1 · M · 의존 없음
- [ ] **C-04** 단일 평가 파이프라인 (`Npc.Eval`): 골든·다양성·실패 집계·비용·다중 모델 대조·게이트 — P1 · L · 의존 C-03
- [ ] **C-05** 검증 실패율 개선 프로그램 (22~32% → ≤10%): 결정론 자동 수선 · few-shot 확장 — P1 · L · 의존 C-04
- [ ] **C-06** 문자열 격리의 테스트 강제 · `reasoning` 정화 · 모더레이션 훅 — P1 · S · 의존 없음
- [ ] **C-07** 스필오버 서브 쿼터 · 버킷/개체 예산 분리 · 프리베이크 전량 운영 절차 — P1 · S · 의존 C-02
- [ ] **C-08** 로컬 추론 프로세스 감독(헬스·재시작) · GPU 메트릭 · 모델 파일 해시 고정 — P2 · M · 의존 A-05
...
- [ ] **F-01** `npc` CLI (`tools/Npc.Cli`): validate/explain/next-code/scaffold/diff/card/timeline/plan/regen — P1 · L · 의존 E-04, F-03
- [ ] **F-02** NPC Studio (웹): 스키마 폼 · 인라인 검증 · 아키타입 카드 · 버킷 그리드 · 플랜 시각화 · LLM 제안 리뷰 — P1 · XL · 의존 E-02, F-03, F-04
- [ ] **F-03** 설명 생성기 `src/Npc.Narrative` (결정론 · LLM 미사용): 아키타입/플랜/인터럽트/인스턴스 → 한국어 카드 — P1 · M · 의존 없음
- [ ] **F-04** 편집 안전장치: 자동 code/bit 할당 · 가중치 재배분 · 파생물 신선도 잠금 · 서식 보존 JSON 편집 · 검증 V12/V13 — P1 · M · 의존 없음
- [ ] **F-05** `ArchetypeCount` 컴파일 상수 제거 (데이터 주도 버킷 공간) — P1 · M · 의존 없음
...
- [ ] **F-08** 에디터 지원: `$schema` 연결 · VS Code 설정 · 스니펫 — P2 · S · 의존 E-02
...
**구현 절차.** `tools/Npc.Eval/`(Prebake·Golden·diversity 코드를 라이브러리화해 참조 — `tools/*.cs` 파일 앱은 `Npc.Eval.Core` 프로젝트로 승격) · 보고서 템플릿 · CI 야간 잡(A-09).

**테스트.** 가짜 엔진으로 파이프라인 종단 · 게이트 판정 로직 · 보고서 스냅샷(시각 없음, 바이트 동일).

**완료 조건.** 두 엔진 대조표가 `docs/measurements/` 에 실측으로 들어가고 `reference_metrics.html` §14 의 "8B 품질 채점" 이 채워진다.
...
**구현 절차.** `src/Npc.Core/Validation/PlanRepair.cs` · `LlmPlanCompiler` 재시도 전에 수선 시도 · `PlanOrigin.Repaired` · few-shot 파일 · `RejectedStore` 에 `repaired` 필드.

**테스트.** `Validation/PlanRepairTests` — 반려 766건을 픽스처로 수선률·재검증 통과율 · 수선이 결정론(같은 입력 → 같은 출력).

**완료 조건.** C-04 회차에서 수선 후 실패율 ≤ 3%.

**규칙 충돌 확인.** §2.6 "검증을 건너뛴 플랜을 런타임에 올리지 않는다" — 수선 후 전 단계 재검증. 재시도 1회 규칙 — 수선은 LLM 호출이 아니다.
...
**구현 절차.** `src/Npc.Llm/LocalEngineProbe.cs` · 설정 필드 · `deploy/compose.yaml` 사이드카 · 문서 라이선스 절.

**테스트.** 프로브 실패 → 브레이커 개방. 해시 불일치 경고.

**완료 조건.** 로컬 엔진을 죽였을 때 60초 안에 알람 + T1 요청이 T2 체인으로 우회(C-01).
