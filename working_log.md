...
## 2026-09-24 01:44 KST · 도입성을 진단하고 보강 계획(T1~T19)을 `ADOPTION_PLAN.md` 로 남겼다

- 깨끗한 클론에서 README 를 그대로 따라 했다 — 빌드 17초·경고 0, 빠른 시작 정상(틱 p99 0.323ms · 할당 0).
  그러나 첫 화면은 버킷 19/2880 · 캐시 히트 0 % · "플랜 스토어가 낡았다" 경고였다.
- `--link record` 트레이스로 결함 둘을 재현했다 — ① 즉시형 명령(`SetVisualState` 등)이 1틱에 끝나 NPC 가
  06:24 에 잠들고 06:25 에 다시 일하러 간다(24분 주기 루프) ② MoveTo `timeout_s 300` × 0.6s/m = 500m 벽 —
  오류 주입 없는 회차에서도 스텝의 13.5 % 가 합성 타임아웃.
- 막는 것 넷으로 정리했다: 라이선스 없음(공개 저장소) · 핵심 약속 결함 · 비-.NET 게임서버가 핸드셰이크 해시를
  얻을 방법 없음 · 문서 과다와 사실 불일치(README "MCP 없음" 등, GitHub 에서 HTML 이 안 읽힘).
- 태스크 19개를 목적·근거·변경 파일·구현·검증·완료 기준으로 적었다. 사람 결정이 필요한 넷(T1·T10·T14·T19)과
  계약 의미 변경 승인(T2·T4)은 문서 맨 위에 모았다. 코드는 건드리지 않았다.
...
- `docs/measurements/conformance_testbed.{md,json}` 이 기본 CI 테스트마다 덮어써져
  `dotnet test` 를 돌릴 때마다 작업 트리가 더러워졌다 — 이벤트 311↔312건, 마지막 틱 242↔243.
- 원인은 회차 길이가 기계 속도에 달려 있던 것이다. ① 접속을 기다리며 틱을 밀던 것을
  틱 없이 기다리게 바꾸고(접속은 `AcceptAsync` 가 제 태스크에서 끝낸다) 재동기화 몫만 고정했다.
  ② 명령 16건이 다 도착할 때까지 다음 틱으로 넘어가지 않게 해 프레임당 최댓값을 고정했다.
- 회차가 `SetupTicks + RunTicks` = 248틱으로 고정됐다. 부하 40개를 띄운 상태를 포함해
  8회 연속 **바이트 동일**을 확인했다. 판정은 C1~C7 전부 그대로다.
- `docs/measurements/` 자체는 git 에서 뺄 수 없다 — Gate·perf 테스트가 읽는 입력이고,
  LLM 비용이 든 실측 원자료다. 문제는 "관리 대상" 이 아니라 **비결정적 회차**였다.

## 2026-09-17 15:32 KST · 시계가 판정하던 테스트 세 자리를 고쳤다

- 전체 실행에서만 빨간불이던 둘(`Handoff_SurvivesConcurrentWorkersAndTicks` ·
  `Worker_T2RunsEightConcurrently`)을 시간이 아니라 **진행**으로 기다리게 바꿨다.
  하나는 30초 토큰이 루프 한가운데서 예외를 던졌고, 하나는 스레드풀 스레드 8개를 재워
  두고 그 위에서 바쁜 대기를 해 정작 워커가 못 올라왔다.
- 검증 중 세 번째가 드러났다 — `Host_LoadsEveryBucketWithinThreeSeconds` 는 저장소에서
  **벽시계를 단언하는 유일한 자리**였다. 배선 검사와 3초 예산을 갈라 예산은 `Category=Load` 로 옮겼다.
  기본 CI 는 이제 기동 시간을 보지 않는다.
- 항상 참이라 아무것도 검사하지 않던 단언(`owner == npc || owner == -1 || owner != npc`)을
  풀이 실제로 약속하는 것으로 바꿨다 — "남의 슬롯에서 플랜이 나오지 않는다".
- **코어 20개에 부하 프로세스 40개를 띄운 상태에서 1,782건 전부 통과**하는 것으로 확인했다.

## 2026-09-17 14:27 KST · X1~X9 를 끝까지 브라우저로 밟고 드러난 결함 넷을 고쳤다

- 앞 회차가 ✅ 로 적어 둔 X2·X5·X7·X8·X9 를 실제로 밟지 않았다는 것을 확인하고 전부 다시 밟았다.
  체크의 뜻을 "코드에 있다" 가 아니라 **"화면에서 봤다"** 로 되돌렸다.
- 결함 넷: ① 새로고침이 편집 폼을 다시 읽지 않았다(세대 가드를 깨우는 사람이 없었다) ②
  "버리고 다시 읽기" 가 초안을 안 버려 확인 모달의 말이 거짓이었다 ③ 다른 탭 배너가 뜬 적이 없다
  (문구를 세운 뒤 `Reload()` 가 지웠다) ④ 검증 위반 41건 전부가 "파일 미상" 이라 바로가기가 죽어 있었다.
- X9 의 "정원 부족으로 만들기 실패" 는 3단계가 먼저 막아 도달할 수 없다 — 의도된 개선이라 두고 기록했다.
- 빌드 경고 0 · 테스트 1,782건 통과 · `dotnet format` 통과. `masterdata/` 는 원상 복구했다.

## 2026-09-17 11:41 KST · Studio 보강 계획(2차) H01~H28 을 전부 구현하고 X1~X9 를 브라우저로 밟았다

- 28건을 구현했다 — 떠날 때 확인 · 되돌리기 트랜잭션 · 낙관적 동시성(지문) · 번호 재배치 차단(`CodePinning`) ·
  막힘 화면의 탈출구 · 생성기 취소 · 표시 이름(V14) 을 Studio 검증에 합류 · 오류 한글화 · UI 용어 검사.
- 로더에 `SkipDistances` 를 넣어 **거리표가 낡아도 화면이 뜬다**. 기본값은 그대로라 `Npc.Host` 기동 경로는 영향이 없다.
- X1·X3·X4·X6 을 브라우저에서 밟으며 화면에서만 보이는 결함 여섯을 더 잡았다 — `default` `ImmutableArray` 패턴 매칭
  NullReference, 새 세부 유형의 표시 이름 누락, V14 힌트가 다른 검사 문장이던 것, 파일 감시가 제 저장을 외부 변경으로
  알리던 거짓 경보, 사는 곳 유형 선택이 비던 것, 미리보기 칸이 한 글자로 눌리던 것.
- 빌드 경고 0 · 테스트 1,781건 통과 · `dotnet format` 통과. `masterdata/` 는 건드리지 않았다.

## 2026-09-17 00:47 KST · Studio 보강 계획(2차) 을 작성했다 — 실수 방지 · 탈출구 · 조용한 파손 28건

- 1차 계획 34건이 "완료" 인 상태에서 Studio 코드 전체 · 매뉴얼 · 테스트 · CODEMAP · ROADMAP 을 다시 대조했다.
  1차가 ✅ 로 표시했지만 코드에 없는 것 일곱 가지(마법사 4단계 편집기 · 장소 지도 클릭 · IssuePanel/`?field=` · 떠날 때 확인 ·
  재생성 확인 모달 · 상황별 탭 숨김 · Ctrl+K)를 찾았다.
- 데이터를 조용히 망치는 길을 찾았다 — 장소 id 중복을 아무도 안 잡고, code 재배치 금지가 문구뿐이며, 연습장 전환이 탭 전역이라
  다른 탭의 저장이 엉뚱한 폴더로 가고, 연습장에서 장소를 추가하면 재시작 말고는 빠져나올 길이 없다.
- 루트에 `STUDIO_IMPROVEMENT_PLAN_2.md` 를 만들었다 — 태스크 H01~H28 · 판정 시나리오 X1~X9 · 확인 모달 문장 규격 · UI 용어표 ·
  오류 한글화 대응표. 1차 문서 머리에 후속 지시서 포인터를 달았다. 코드는 건드리지 않았다.

## 2026-09-16 23:11 KST · Studio 개선 계획 34건을 끝내고 S1~S6 를 브라우저에서 판정했다

- 계획서의 남은 태스크(T19 매뉴얼·README 재작성, T20 테스트 정비) 를 끝내고 34건 전부 체크했다.
- 매뉴얼을 "화면 구성 → 기능 목록" 에서 "목표 → 따라하기" 로 다시 썼다. 1~7장은 JSON 을 한 번도 보지 않고 따라간다.
- 판정 시나리오 S1~S6 를 브라우저에서 직접 밟았고, 그 과정에서 결함 넷을 찾아 고쳤다 — 순찰로 없는 NPC 개요 NullReference,
  검증·진단 화면이 첫 렌더에서 죽던 것, 장소 추가 뒤 화면이 통째로 죽던 것, 마법사에서 배치 예측이 일터 선택을 안 따라오던 것.
- 진단은 출하 데이터에서 19건을 잡았다(L7 18 · L6 1). `masterdata/` 는 고치지 않고 계획서에 기록만 했다.

## 2026-09-16 21:40 KST · Studio 화면 전체를 다시 만들었다 (T05~T18 · T21 T24~T28 T30~T34)

- 읽기(개요·NPC·장소·마을 지도·돌발 반응·행동 카탈로그), 예측(하루 재생·상황 반응·버킷 격자·라이브 관찰),
  편집(아키타입 폼·하루 일과 폼·새 장소 폼·새 직업 마법사 5단계), 안전(검증 바로가기·파급 패널·되돌리기) 을 넣었다.
- 무변경 저장은 바이트 동일이다. 바뀐 필드만 제자리에서 고치고 들여쓰기·키 순서를 원문에서 읽는다.
- `JsonSurgeon` 에 최상위 속성 추가·삭제를, `Npc.Core` 에 `BucketKey.TryParse` 를 더했다.
- 마법사는 `pois.json` 의 작업 권한까지 같은 트랜잭션에 넣는다 — 안 넣으면 `V3.UNREACHABLE_POI` 로 막힌다.

## 2026-09-16 20:50 KST · 정의만으로 하루를 예측하는 엔진을 코어에 넣었다 (T05~T07 · T22 T23 T29 T33 T34)

- `DayForecast` — 정의에서 24시간 위치·행동·상태를 계산한다. `ReactionForecast` — 상황별 인터럽트 반응과 그 이유.
- 소요 시간 계산을 `ActionDuration` 한 벌로 내리고 `SimWorld` 가 그것을 부른 뒤 지터를 얹는다.
  심볼 바인딩도 `PoiTable.NearestEnterable` 한 벌이다 — **드리프트를 테스트가 아니라 구조로 막았다.**
- `ArchetypeLint` (L1~L10) · `PlacementForecast` · `DefinitionDiff` 와 `npc forecast` · `npc lint` 명령.
- 카드 출력은 바이트 단위로 불변이다 — 골든이 그것을 지킨다.

## 2026-09-16 20:30 KST · 시작 화면·도움말 서랍·연습장을 추가했다 (T03 T04 T30)

- 시작 화면에 마을 한눈에(직업·NPC·장소·지역) · 할 일 카드 4장 · 검증/해야 할 일/연습장/용어를 넣었다.
- 필드 옆 ⓘ 툴팁(`FieldInfo`)과 화면별 도움말 서랍을 붙였다. JS 없이 CSS 팝오버다.
- 연습장(T30): 원본을 통째로 복사한 `lab/studio-<이름>/masterdata` 에서 편집하고, 원본과 비교·적용·버리기를 한다.
- 파생물 재생성은 연습장에서 막았다 — 생성기가 입력 디렉터리 인자를 받지 않아 원본을 읽기 때문이다.
- `Sandbox_CopiesAndIsolatesWrites` 가 원본 바이트 불변을 강제한다. Studio 테스트 10건 통과.

## 2026-09-16 20:05 KST · 사전을 코어에 넣었다 — 액션·직군·필드 설명 (T02)

- `Lexicon` 에 액션 37종 한국어 표기, 직군 7종(`ArchetypeGroup`)과 직군 색, 로컬라이즈 우선 표시 이름을 더했다.
- `FieldGuide` 를 새로 만들었다 — 마스터데이터 13개 파일의 필드 140개에 "무엇·왜·주의·근거" 를 붙였다.
- 드리프트 테스트 5건: 액션 전수 · 직군 전수 · `docs/schema` 속성 전수 · 빈 문장 금지 · 로컬라이즈 우선.
- 아키타입 카드는 그대로 둔다 — 골든과 블라인드 평가 자료의 전제가 액션 id 그대로다.
...
- 초보자가 어려운 이유 15가지를 코드 원인과 함께 진단했다. 핵심 빈틈은 "정의를 읽어도 어떻게 움직일지 모른다" 였다.
- 2차 개정에서 **동작 예측**(F 단계 T22~T29) 을 더했다 — 정의만으로 24시간 위치·행동·상태를 계산하는 `DayForecast`, 상황 반응과 이유를 내는 `ReactionForecast`, 지도 위 SVG 인형 재생 화면, 버킷별 미리 구운 플랜 격자, 라이브 관찰. 소요 시간·심볼 바인딩·인터럽트 매칭이 전부 코어에 있음을 확인해 부록 D 에 수식으로 적었다.
- 연습장·따라하기·전역 검색·건강 진단·저장 전 요약(T30~T34) 과 SVG 인형 규격(부록 E) 도 더했다. 구현은 다음 세션부터다.

## 2026-09-16 17:57 KST · 순찰 경로 POI 선택기를 추가했다

- 같은 지역의 POI를 유형·세부 종류·정원·좌표와 NPC의 집·일터 관계로 설명하도록 했다.
- 최대 4개 장소를 추가하고 방문 순서를 바꾸거나 제거하는 순찰 경로 편집 UI를 구현했다.
- POI ID 직접 입력은 접힌 고급 기능으로 유지하고 README와 HTML 매뉴얼을 갱신했다.
- 브라우저 동작 검증과 Release Studio 빌드, 1,714개 테스트, 포맷 검사를 통과했다.

## 2026-09-16 17:15 KST · 설명 마크다운 렌더링과 개별 NPC 편집을 완성했다

- 아키타입 실시간 설명과 설명 카드, 개별 NPC 카드를 안전한 HTML로 렌더링하도록 했다.
- 개별 NPC의 순찰·공격 반경·세력·대화·일정 오버라이드를 폼에서 저장·삭제하도록 했다.
- 읽는 NPC 정의 파일과 기존 `npc_overrides.json` 갱신 규칙을 README와 HTML 매뉴얼에 명시했다.
- 포트 25056 브라우저 검증과 전체 Release 빌드, 1,714개 테스트, 포맷 검사를 통과했다.

## 2026-09-16 16:39 KST · NPC 그룹 탐색과 아키타입 실시간 설명을 추가했다

- 개별 NPC 5,000명을 아키타입→지역으로 묶고 ID·직업·지역·장소 검색과 오버라이드 필터를 제공했다.
- 아키타입 JSON 초안을 저장 전에 해석해 설명 카드가 400ms 디바운스로 갱신되도록 했다.
- Studio 기본 포트를 25056으로 통일하고 README와 HTML 매뉴얼의 실행·사용 절차를 갱신했다.
- 브라우저 검증과 전체 Release 빌드, 1,712개 테스트, 포맷 검사를 통과했다.

## 2026-09-16 14:57 KST · NPC Studio GUI와 사용 매뉴얼을 구현했다

- 아키타입·개별 NPC 카드 조회, 원본 JSON 편집, 새 직업 생성 기능을 독립 Blazor 웹 도구로 추가했다.
- 저장 전 임시 작업본에 V1~V13·실제 로더·NPC 명단 검증을 적용하고 실패 시 원본을 보존하도록 했다.
- HTML 매뉴얼과 README·코드 지도를 갱신하고 브라우저에서 주요 화면과 읽기 전용 잠금을 검증했다.
- 전체 Release 빌드와 1,710개 테스트, 포맷 검사, 마스터데이터 검증을 통과했다.

## 2026-09-16 13:09 KST · LLMNPCServer 소개 인포그래픽 4종을 제작했다

- 프로젝트의 정체성, 장점, 빠른 체험, 올바른 사용 원칙을 각각 4:5 세로형 한국어 인포그래픽으로 구성했다.
- README와 테스트베드 문서를 근거로 5,000 NPC·10Hz, LLM 장애 대응, 캐시 재사용률, 실제 실행 명령을 반영했다.
- 생성 결과를 육안 검수하고 데이터 흐름, PowerShell 실행 명령, 중복 번호를 교정했다.
- 최종 PNG 4개를 `docs/infographics/`에 저장했다.

## 2026-09-16 12:39 KST · 개발 중에만 쓸모 있던 문서를 지우고 합쳤다

"문서가 많으면 오히려 불편하다" — 이제 필요 없는 개발용 문서를 적극적으로 정리해 달라는 요청.

- **`LLM_NPC_Server_Plan.md` 삭제** (54KB). 구현 **전** 타당성 판단서다. 판단 근거(§2~§4·§10~§11)는
  `docs/book/ch01.html` 이 그대로 대체했고, 비용·모델 후보는 `reference_metrics.html`, 연동·마스터데이터는
  `reference_link/masterdata.html` 로 갔다. 12주 로드맵·W1 스파이크 체크리스트·v2/v3 개정 이력은 끝난 과거이고,
  §16 문서 맵은 네 번째 사본이었다. 이미 삭제된 `docs/measurements/W1_results.md`·`cost_actual.md` 를
  가리키고 있었다. 리스크 대장 R1~R7 도 전부 `book/ch01` · `docs/legal/` · `docs/security/` 로 흡수됐다.
- **`PRODUCTION_ROADMAP.md` 2,450줄 → 315줄** (217KB → 30KB). 50건 중 44건 완료. 완료분의 착수 전 상세
  (현재 진단 · 설계 · 구현 절차 · 테스트 · 완료 조건, 태스크당 60~90줄)는 코드가 대체한 지시문이라 지웠다.
  **무엇을 왜 했고 무엇이 설계와 달라졌는지는 §1 체크리스트 한 줄에 그대로 있다.** 낡아서 거짓이 된
  §2 마일스톤 · §3 현재 상태 진단(2026-09-09 기준이라 "없음" 이 44건 분량 틀렸다) · 부록 A 근거 파일 인덱스도
  지웠고, 부록 B 용어는 `docs/llm/GLOSSARY.md` 로 넘겼다. E-05 의 "근거는 아래" 는 사라질 절을 가리키므로
  이유를 §1 줄 안으로 옮겼다.
- **`AGENTS.md` 6.2KB → 2.1KB.** §5.1 이 `CLAUDE.md` §5.1 과 글자 단위로 동일했고 §1~§4 도 §4·§7 의 재서술이었다.
  포인터 표만 남겼다 — 규칙을 두 벌로 두면 반드시 어긋난다.
- 코드 주석 17곳의 `PRODUCTION_ROADMAP §4` 류 참조에서 절 번호를 뺐다. 절 번호가 바뀌어 **다른 절을 가리키게**
  됐기 때문이다. 태스크 id(`A-01`)는 안 바뀌므로 그것만 남겼다.
- 문서 지도 5곳(`CLAUDE.md` · `README.md` × 3 · `docs/index.html` · `docs/book/index.html` ·
  `docs/tutorial/ch00.html` · `docs/book/ch01.html` 출처 칩) 갱신. 끊긴 링크 0건 확인.

검토했으나 **남긴 것**: `docs/book`(13장)과 `docs/tutorial`(21장)은 독자가 다르고 중복이 아니다.
`docs/FAQ.html`(도입 검토)과 `docs/index.html` §17(트러블슈팅)도 다르다. `docs/llm/**` · `security/**` ·
`legal/**` · `wire/layout_v2.md` · `schema/**` 는 테스트가 존재와 내용을 강제하고 MCP 리소스로 나간다.
`docs/measurements/runs/*.jsonl` 은 문서가 아니라 실측 원자료라 `CLAUDE.md` §6 이 보호한다.

빌드 경고 0 · 오류 0 · 테스트 1,705건 전부 통과 · `dotnet format` 통과.


## 2026-09-13 11:25 KST · README.md 를 사용자 가이드 / 개발자 가이드로 분리했다

"서버 개발자"와 "서버를 실행해서 쓰는 사람"을 나눠 달라는 요청. 사용하는 사람이 개발
관련 정보를 안 봐도 되게 하는 것이 목적.

- 앞부분(이게 뭔가요·기능 목록) 다음에 **"어디부터 읽어야 하나"** 안내 표를 넣어
  두 갈래로 바로 갈라지게 했다.
- **사용자 가이드**: 준비물 → 빌드 → LLM 없이 실행 → 규모 키우기 → 게임서버 연결 →
  LLM 켜기 → Docker/K8s 배포 → LLM 에이전트용 지시문까지, 번호 붙은 짧은 절차로 재배열했다.
  기존에 흩어져 있던 실행 옵션 전체 표·환경변수·관리 API curl 예시는 `<details>` 로
  접어서 필요할 때만 펼쳐보게 했다 (평소엔 안 보인다).
  - 접는 과정에서 원래 있던 문서 버그도 고쳤다: "주요 실행 옵션" 표가 관리 API 코드
    블록에 의해 중간에 끊겨서 `--max-speed`·`--gs-host`·`--shard` 등 뒷부분 행이
    헤더 없는 고아 행으로 떠 있었다. 하나의 완결된 표로 합쳤다.
- **개발자 가이드**: 프로젝트 구조·핵심 아이디어(아키텍처 다이어그램)·`npc` CLI·
  "MMO 개발에 LLM 을 붙일 때"(에이전트 지시문)·로드맵·성능 목표·문서 전체 지도를
  전부 이 아래로 옮겼다. 서버를 실행만 하는 사람은 이 절 전체(문서 후반부 절반 이상)를
  안 읽어도 된다.
- 라이선스 주의는 공통이라 문서 맨 끝에 그대로 남겼다.

## 2026-09-12 18:45 KST · README.md 를 사용자 관점으로 다시 썼다 — 기능 목록·서두·사용법

기능 목록이 구현 세부사항(SoA·힙 할당·Volatile 등) 위주라 "무슨 기능이 있는지" 알기
어렵다는 지적을 반영했다.

- 서두("이게 뭔가요?")를 새로 써서 목적을 4문장 안에 설명하도록 했다. 기존 P0~P6 게이트
  숫자 나열은 로드맵 표·`reference_metrics.html` 참고로 축약했다.
- **기능 목록**을 전면 재작성했다. 구현 용어 대신 "무엇을 해주는가" 문장으로 바꾸고,
  런타임/플랜/연동/운영/콘텐츠 구분 대신 사용자 체감 단위(행동·가용성·연동·비용·콘텐츠·운영)로 재분류했다.
- **"이 서버를 실제로 쓰려면"** 절을 새로 추가했다 — 게임 서버에 붙이는 개발자, 콘텐츠
  담당자, 그리고 **"LLM 에게 NPC 서버로 실행시켜 쓰라고 시킬 때"** 줄 지시문을 구분해 넣었다.
  기존 "MMO 개발에 LLM 을 붙일 때" 절(콘텐츠 편집 에이전트용)과는 대상이 다름을 명시했다.
- 기존 표(무엇을 만들고/안 만드는가·핵심 아이디어·결과물·요구 환경·빠른 시작 이후 전부)는
  손대지 않았다 — 이번 요청은 서두와 기능 목록의 이해도 문제였다.
