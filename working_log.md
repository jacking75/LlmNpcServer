...
## 2026-09-12 12:44 KST · 불안정한 테스트를 고쳤다 — 시간 의존 4건 · 할당 측정 24건

가끔 빨간불인 테스트는 사람이 전체 결과를 안 믿게 만든다 (CLAUDE.md §5.1).

- **하트비트 상한 2초 → 30초.** 이것이 TCP 링크 테스트 흔들림의 진짜 원인이었다 —
  전체 스위트를 병렬로 돌릴 때 호스트가 잠깐 굶으면 세션이 죽은 것으로 판정돼
  **계획에 없던 Degraded → 재접속**이 끼어들었다. 상한이지 지연이 아니라 happy path 는 그대로다.
- **맨 `TaskCanceledException` 을 없앴다.** `WaitAsync`·`WaitForSessionAsync` 가
  무엇을 기다리다 못 봤는지 적고 실패한다. 전에는 진짜 회귀와 느린 기계를 구별할 수 없었다.
- **`Worker_RunsOutsideTickLoop`** 의 안쪽 시한 10초를 바깥 cts(30초)에 맞췄다.
  이 테스트는 풀 스레드 둘을 동기로 붙잡는데, 풀은 최소치를 넘으면 초당 한둘씩만 늘린다.
- **`Reload_DoesNotAllocateInTheTickLoop`** 의 회전수 상한(10,000)을 시각 상한으로 바꿨다.
  풀이 굶으면 호스트가 첫 틱을 돌리기도 전에 회전이 다 타서 리로드가 0회가 됐다.
- **할당 측정 24곳을 전부 `AllocationProbe` 로 옮겼다.** 계층 JIT 재컴파일이 측정 창에
  떨어지면 코드가 아무것도 안 해도 수 KB 가 잡힌다. 계기는 이미 있었는데 한 곳만 쓰고 있었다.
  예외는 `TickAllocationTests` 하나 — gen0 카운트를 같이 보고 병렬이 꺼진 컬렉션에 있다.
  이유를 주석에 적었다.
- **비동기판 계기는 만들었다 지웠다.** `Func<Task>` 를 창 안에서 await 하면 상태 기계가 할당돼
  **계기가 자기 자신을 잰다**(실측 136B). 대신 "Flush 가 동기로 끝나는가" 를 단언하고
  동기 구간만 잰다 — 틱 루프 안에서 Flush 가 비동기로 넘어가면 그것이 곧 결함이라
  오히려 더 센 단언이다.
- 전체 스위트 5회 연속 **1,705 통과 0 실패**. 부분 필터 회차도 5회 연속 통과.
