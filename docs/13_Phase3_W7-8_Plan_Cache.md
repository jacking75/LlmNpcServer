...
### 항목 2·3·4 의 판정선 — "전량" 에서 "선언한 범위 완주" 로 (결정 15-A)

원래는 `생성/2,880 ≥ 95%` 여야 판정했다. 그 방어의 목적은
**"288버킷 파일럿의 115초를 상한 300초 통과로 세지 않는다"** 였고 그 목적은 지금도 옳다.
다만 판정선을 전량에 묶어 두면 도달 집합만 만드는 방침에서 **영원히 판정할 수 없다.**

그래서 묻는 것을 바꾼다 — **"이 회차가 만들기로 한 것을 다 만들었는가"** 다.

```
RequireCompleteRun:  counts.target > 0   (스키마가 신형인가)
                  && !partial            (예산 캡에 걸려 중단되지 않았는가)
```

**대신 측정치를 범위 없이 읽지 못하게 한다.** 항목 2·3·4 의 실패 메시지는 항상
`[대상 N버킷 · 스토어 M/2880]` 을 같이 찍는다 — "97초" 가 264버킷의 97초라는 것을
읽는 사람이 놓치면 게이트가 다시 거짓이 된다.

> **`counts.target` 은 2026-07-28 에 추가됐다** (`docs/03 §7`). 그 이전 manifest 는 이 값이 0 이라
> 항목 2·3·4 가 **"프리베이크를 다시 돌려야 판정할 수 있다"** 로 실패한다. 의도한 동작이다 —
> 범위를 모르는 숫자를 통과로 세지 않는다.
