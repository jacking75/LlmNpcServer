...
### 일일 캡은 T2 에만 걸린다 (T4-06)

| 상한 | T1 | T2 |
|---|---|---|
| 초당 요청 | **적용** (GPU 용량) | **적용** (rate limit) |
| 일일 토큰 | 세지만 막지 않는다 (`LocalTokensToday`) | **적용** |
| 일일 비용 | 항상 $0 | **적용** |

**T1 토큰까지 캡으로 막으면 `CLAUDE.md §2.7` 의 "T2 → T1 강등" 이 성립하지 않는다** — 캡을 넘은 순간 T1 도 같이 막혀 곧바로 거절이 되고, P4 게이트 항목 7("일일 토큰 캡 초과 시 T2→T1 자동 강등")을 통과할 수 없다. 토큰 캡은 **비용의 대리 지표**이고 로컬 토큰에는 대응하는 비용이 없다. T1 의 상한은 초당 요청(= GPU 시간)이다.

강등 체인은 `T2 → T1 → None` 뿐이고, 거절은 실패가 아니다 — 그 NPC 는 기존 플랜을 계속 쓰고 다음 인지 스캔에서 다시 큐에 들어온다. 캡 초과·강등·거절은 전부 `ReplanBudget.Alarm` 으로 나가고 호스트가 로거·대시보드(T4-20)로 올린다.
...
| 지표 | 기대 | **T4-16 실측** (`W10_scale.md`) |
|---|---|---|
| 틱 시간 | O(n) — 실행기가 전체 순회 | O(n) (기울기 1.18) |
| 인지 스캔 | O(1) — 슬라이스가 고정 비율이므로 | ⚠ **상한을 풀면 O(n)** (기울기 1.04) |
| LLM 요청 | **O(1)** — 예산 상한에 걸림 | O(1) — 예산에 NPC 수가 인자로 안 들어간다 |
| 명령 송출 | O(n) | O(1)로 보임 — `--max-speed` 회차라 **벽시계 처리량**이다 |
| 메모리 | O(n) | 준상수 — 힙에 마스터데이터·플랜이 크게 깔린다 |

### ⚠ "인지 스캔 O(1)"의 근거가 틀렸다 (T4-16 실측)

상한을 푼 회차의 실측이다 (`--scan-cap 0`, 봇 20, 배속 600):

| NPC | 500 | 1,000 | 2,500 | 5,000 |
|---|---:|---:|---:|---:|
| 틱당 스캔 | 58 | 98 | 279 | **637** |

NPC 10배에 스캔 **11배**다. `docs/11 §4`의 "상한이 없으면 612까지 간다" 추정과 4% 차이로 맞는다.

**"슬라이스가 고정 비율이므로 O(1)"은 성립하지 않는다.** 슬라이스는 밴드 인원을 `Period`로 나눈 것이고, **밴드 인원이 NPC 수와 함께 자란다.** 플레이어 봇 수는 고정이지만 월드(존·POI)가 고정이라 NPC를 늘리면 밀도가 올라가 한 플레이어 반경 안의 NPC가 그만큼 늘어난다.

**실제로 O(1)을 만드는 것은 `CognitionScheduler.MaxScansPerTick`(150)이다.** 밴드별 커서가 잘린 뒤쪽을 다음 틱에 이어 보므로 굶는 NPC는 없다. 즉 **설계는 맞고 이 표의 근거가 틀렸다** — 상한은 성능 안전장치가 아니라 **차수를 결정하는 본체**다. `docs/11 §4`가 "슬라이스만으로는 상한이 안 지켜진다"고 이미 같은 말을 하고 있었다.

`Scale_CognitionIsConstant`는 "밴드 인원이 인구에 비례하지 않는다"는 전제(docs/11 §4의 등급 분포)가 성립할 때 O(1)임을 확인하고, `Scale_CognitionGrowsWhenBandsGrowWithPopulation`이 그 전제가 깨질 때 O(n)이 됨을 **반증으로** 확인한다.

> 상한이 걸린 회차는 "굶는 NPC 없이 잘리는가"를 보는 용도다 — 그 회차에서 차수를 재면 무엇을 넣어도 150에서 잘려 O(1)로 보인다.

### ⚠ NPC 10,000(스트레스) 축은 아직 측정되지 않는다

`masterdata/npc_instances.json`에 5,000마리뿐이라 `--npcs 10000`은 실제로 5,000을 돌린다 (`NpcHost.Create`의 `Math.Min`). `tools/gen_npcs.cs`의 `Population`이 5,000 고정이다 (`docs/01 §9`). CSV에 `npcs_actual` 열을 두어 이 차이가 조용히 넘어가지 않게 했다.
...
- [ ] 틱 루프 Gen0 GC = 0 · **정상 상태 틱 창 할당 = 0** (아래 각주)

> **항목 9 는 "누계 0" 이 아니라 "정상 상태에서 0" 이다** (2026-07-28 정정).
> 누계로 재면 콜드 스타트를 함께 센다 — JIT · 정적 초기화 · 첫 인터페이스 디스패치는
> 그 코드 경로가 **틱 창 안에서 처음 실행될 때** 몇십 바이트를 내고, JIT 런타임에서
> 그것을 0 으로 만들 수는 없다.
>
> 실측: 게임 **3일(4,320틱)** 회차에서 할당이 있었던 틱은 **1·148·396 셋뿐**이고 합계 **264 B** 다.
> 게임 1일(1,440틱) 회차의 합계도 **같은 264 B** — 회차를 3배 늘려도 늘지 않으므로
> **틱당 할당은 실제로 0** 이다.
>
> 그래서 판정은 **"회차 후반부에 할당이 하나라도 있으면 실패"** 다. 진짜 누수는 계속 나므로
> 반드시 후반부에도 걸린다. **임의의 워밍업 상수를 두지 않는다** — 그 값을 실측에 맞춰 정하면
> 게이트를 실측에 맞추는 것이 된다.
