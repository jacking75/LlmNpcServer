...
**발동은 엣지 트리거다.** `when`의 조건은 대부분 플래그라 한 번 참이 되면 한동안 참으로 남는다 — `go_home_at_night`은 집에 닿을 때까지, `rest_when_exhausted`는 스태미나가 찰 때까지 참이다. 들어오는 이벤트마다 다시 발동시키면 `강제 액션 → 완료 이벤트 → 다시 강제`의 되먹임이 생겨 명령 수가 NPC당 수천 건으로 폭주한다.

그래서 매처는 NPC별로 **마지막에 발동한 규칙을 기억하고, 같은 규칙이 이어서 걸리면 넘긴다**. 조건이 한 번 풀렸다가(= 그 NPC에 어떤 규칙도 안 걸리는 이벤트가 한 번 오면) 다시 참이 될 때 재무장한다. `cooldown_s` 같은 필드를 두지 않는 이유는 시간 기반 쿨다운이 `Tick`에 의존해 리플레이 재현을 어렵게 만들기 때문이다 — 엣지 트리거는 상태만 보므로 결정론이다.
...
```
[1] system_rules.md                             (905 tok)
[2] actions.json + items.json → 액션 카탈로그   (6,052 tok)  ← 자동 생성
    (액션별 플래그 5종 표 · 레시피 입출력 포함)
[3] archetypes.json → 아키타입별 허용 액션 표   (2,392 tok)  ← 자동 생성
[4] 03_PlanDSL_Spec의 스키마 요약 + 스키마 본문 (1,718 tok)  ← 자동 생성
[5] fewshot/*.json 3건                            (900 tok)
────────────────────────────────────────────────────
합계 실측: 11,967 토큰 (T2-04, o200k_base. docs/measurements/W6_compile_stats.md §1)
```

**4,096 토큰 이상이어야 한다.** Gemini 3.x와 Claude Haiku 4.5의 프롬프트 캐시 최소 임계가 4,096이다. 미달하면 캐시가 아예 안 걸린다.

> **상한은 없다.** 4,200~4,500 이라는 초기 예산은 액션 12·플래그 16 짜리 축소판(W1 스파이크 4,409 tok)으로 잡은 값이었다. 정식 카탈로그는 액션 37·플래그 42 이고, 여기에 통과율 처방 셋(액션별 플래그 표·레시피 입출력·아키타입 허용 액션)이 더 붙는다. **상한을 지키려고 카탈로그를 줄이지 않는다** — 캐시에 필요한 것은 하한뿐이고 상한은 비용 문제일 뿐이다. 세 처방을 서픽스가 아니라 여기에 두는 이유는 서픽스 예산이 300 토큰이기 때문이다 (`docs/12 §3`).
