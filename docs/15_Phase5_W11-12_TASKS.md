...
```
[x] T2 차단 → T1 페일오버, 지연 상승하나 동작 유지
[x] T1까지 차단 → 캐시 플랜만으로 5,000 NPC 정상 동작
[x] PlanStore까지 차단 → 폴백 40개만으로 정상 동작
[x] 세 단계 모두 크래시 0 · 틱 시간 예산 내
```

> **실측 (2026-07-27):** 5,000 NPC · 18,720틱(게임 13일) · p99 0.367ms · 오버런 0 · 시퀀스 갭 0.
> "T2 페일오버 시 지연 상승" 은 이 회차에서 관측되지 않는다 — `--tier none` 으로 돌려
> 런타임에 LLM 티어가 없었고, T1·T2 차단은 라우터 단위 테스트(`TieredPlanCompilerTests`)로만
> 확인된다. 이 회차에서 실제로 끊긴 것은 `PlanStore` 하나이고, 그 단계는 히트율로 관측된다.
>
> **P4 병합 후:** 킬스위치는 `TierWiring.Build` 를 통해 티어 라우터에도 결선돼 있다
> (`Program.cs` → `TieredPlanCompiler.Switches`). 티어를 켜고 이 회차를 다시 돌리면
> T2 → T1 페일오버가 실측으로 관측된다 — 아직 돌리지 않았다.
...
> 1번은 `LoopbackGameServerLink` 로 확인할 수 없다 — 인프로세스라 언제나 Connected 이고
> `Faulted` 는 미구현인 `TcpGameServerLink` 의 경로다. 계약을 만족하는 최소 구현으로
> "축적하고 버리지 않고 순서를 지킨다"를 고정했다.
> 5번의 서킷 브레이커는 T4-09 의 산출물인데 P4 가 미착수라 여기서 먼저 만들었다.
...
```
[x] 골든 50건 합격률 ≥ 90%              93.6 % (206/220)
[x] 결정론 리플레이 100% 일치           기록 2회 바이트 동일 · 재생 명령 열 동일
[x] 시나리오 A / B / C 전부 통과        셋 다 완주 · 오버런 0 · 갭 0
[x] 링크 4종 교체 시 런타임 코드 diff = 0  런타임 3프로젝트가 Npc.Gateway 미참조
[-] 블라인드 평가 n ≥ 480               n = 0. 자료·양식·분석기는 준비, 참가자 0명
[~] 오써링 공수 축 A·축 B 양쪽 산출     축 B 확정(72배) · 축 A 는 가정 기반
[x] 비용 실측과 추정의 오차율 제시      +445 %(프리베이크) · +212 %(월 운영)
[x] docs/00 §4 수용 기준 전 항목 체크 완료  16개 전부 판정 (9 통과 · 7 미달)
[x] 보고서 작성 완료                    docs/RnD_Report.md 8개 절
```

> **6/9 통과 · 1 부분 · 2 미달.** 판정 근거는 [`measurements/P5_gate.md`](measurements/P5_gate.md).
> **미달은 전부 "아직 안 돌렸다" 다** — 코드 결함으로 미달한 항목은 하나도 없다.
> 블라인드 평가는 사람 12명이, 축 A 는 수작성 실측(T1-54 재실행)과 검수(T3-18)가 필요하다.

---

## E. P4 병합 후 후속 (W12+)

**T5-21** 메꾼 티어가 킬스위치를 물려받게 한다 · `S` · 선행 T5-08, T4-07
  파일 `src/Npc.Llm/TieredPlanCompiler.cs` (수정), `src/Npc.Host/Replan/TierWiring.cs` (수정), `tests/Npc.Tests/Llm/TieredPlanCompilerTests.cs` (수정)
  사양 `docs/15 §4` · `docs/14 §4`
  내용 **단일 티어 구성에서 킬스위치가 아무것도 끊지 않는다.** `TierWiring` 은 한쪽 티어의 엔진이 없으면 없는 쪽을 있는 쪽으로 메꾼다 (`new TieredPlanCompiler(t1 ?? t2!, t2 ?? t1!, ...)`) — 강등·페일오버가 같은 엔진으로 가게 하려는 의도다. 그런데 `--tier t2` 로 돌리면 local 자리에 T2 컴파일러가 들어가고, T2 킬스위치가 발동하면 `SelectTier` 가 T1 으로 강등하는데 **그 T1 이 실은 T2 엔진이라 끊은 티어가 계속 불린다.**
  내용 `KillSwitchTarget` 의 주석이 금지한 상태다 — "안 끊긴 채로 통과하면 게이트가 거짓이 된다". `--tier all` 에서는 드러나지 않고, **Poe 등 외부 엔진 하나로만 시나리오 C 를 돌릴 때 정확히 이 구성이 된다.**
  내용 원인: T5-08 의 첫 구현은 컴파일러를 nullable 로 받아 `Available` 에서 `_t2 is not null` 을 봤는데, P4 병합에서 T4-07 의 non-null ctor 를 택하면서 그 방어가 빠졌다.
  내용 규칙 — **메꾼 티어는 실제 티어의 차단을 물려받는다.**

```
Available(T1) = !cut(T1) && (HasT1 || !cut(T2))
Available(T2) = !cut(T2) && (HasT2 || !cut(T1))
```

  내용 `HasT1`·`HasT2` 는 `init` 기본값 `true` 로 둔다 — 그러면 기존 호출부와 테스트가 그대로 통과하고, `TierWiring` 만 `HasT1 = t1 is not null` 을 넘긴다.
  완료 `--tier t2` 에서 T2 차단 → 전면 거절 (외부 호출 0건)
  완료 `--tier t1` 에서 T2 차단 → 진짜 T1 으로 페일오버
  완료 `--tier all` 은 판정이 지금과 동일 (회귀 없음)

> **시나리오 C 를 티어 켜고 다시 돌리기 전에 반드시 끝낸다.** 이게 없으면 T5-09 의
> 1·2단계가 "끊었는데도 외부 API 가 불린 회차" 를 통과로 기록한다.
...
```
W11 ── 골든 · 결정론 · 장애
[x] T5-01 픽스처포맷+단언8종 [x] T5-02 단언평가기      [x] T5-03 픽스처50건 ★
[x] T5-04 골든러너(3중2)     [x] T5-05 난수봉인감사    [x] T5-06 시간봉인감사
[x] T5-07 리플레이일치 ★     [x] T5-08 KillSwitch      [x] T5-09 시나리오C ★
[x] T5-10 링크장애주입       [x] T5-11 링크4종교체 ★

W12 ── 평가 · 정산 · 보고
[x] T5-12 Narrate            [x] T5-13 평가자료40건    [x] T5-14 응답수집양식
[x] T5-15 통계처리 ★         [x] T5-16 오써링정산 ★    [x] T5-17 비용정산
[x] T5-18 버킷히트맵리포트   [x] T5-19 R&D보고서 ★     [x] T5-20 최종게이트
...
★ T5-21은 **시나리오 C를 티어 켜고 돌리기 전의 선행 조건**이다.
