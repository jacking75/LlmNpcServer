...
**T5-01** 골든 픽스처 포맷 + 단언 종류 · `M` · 선행 T2-13
  ...
  사양 `docs/15 §2`
  내용 §2의 단언 8종(`validates`/`contains_action`/`not_contains`/`produces_item_of`/`ends_with_any`/`step_count_between`/`acquires_before_use`/`avoids_flag_while`/`differs_from`). **속성 단언만. 정확한 문자열 비교 금지.**
  완료 8종 각각 통과/실패 케이스 테스트
...
**T5-04** 골든 러너 · `M` · 선행 T5-03
  ...
  완료 `Category=Golden` 테스트가 합격률 ≥ 90% · 실행 비용 기록
...
**T5-05** 난수 봉인 감사 · `M` · 선행 T1-62
  ...
  내용 `Npc.Core`/`Runtime`/`Planning` 어셈블리에서 `System.Random` 사용 검출. 시드 없는 사용 0.
  완료 `Determinism_NoUnseededRandom` 통과 · 발견된 위반 전부 `(npcId, tick)` 해시로 교체
...
**T5-06** 시간 봉인 감사 · `S` · 선행 T5-05
  ...
  완료 `Determinism_NoWallClock` 통과 (계측 전용 경로는 화이트리스트)
...
**T5-08** `KillSwitch` 이벤트 처리 · `S` · 선행 T1-52, T4-07
  파일 `src/Npc.Sim/ScenarioRunner.cs` (수정), `src/Npc.Llm/TieredPlanCompiler.cs` (수정)
  사양 `docs/02 §5` · `docs/15 §4`
  내용 `KillSwitch{target: T1|T2|PlanStore}` 수신 → 해당 티어 비활성.
  완료 3종 타깃 각각 비활성 확인
...
**T5-11** 링크 4종 교체 검증 · `M` · 선행 T5-07, T1-45
  ...
  내용 Null/Loopback/Recording/Replay 4종으로 같은 시나리오 실행.
  완료 `Link_Swappable` 통과 · **`src/Npc.Runtime`, `src/Npc.Core`, `src/Npc.Planning` 변경 diff = 0** · `TcpGameServerLink`가 컴파일됨
...
**T5-12** `Npc.Narrate` — 로그 → 서술 · `M` · 선행 T5-07
  파일 `tools/Npc.Narrate/Program.cs` (신규)
  사양 `docs/15 §6` 서술 변환 예시
  내용 명령 로그 → 사람이 읽는 하루 일지. **액션 시퀀스의 기계적 번역이므로 LLM을 쓰지 않는다.**
  완료 §6 예시 형식으로 출력 · 어느 쪽(LLM/사람)이 만든 플랜인지 서술에서 드러나지 않음

**T5-13** 평가 자료 생성 · `M` · 선행 T5-12
  파일 `tools/gen_blind_eval.csx` (신규), `artifacts/blind_eval/*.md` (산출 40건)
  사양 `docs/15 §6`
...
**T5-15** 통계 처리 · `M` · 선행 T5-14
  파일 `tools/analyze_blind_eval.py` 또는 `.csx` (신규), `docs/measurements/blind_eval_result.md` (산출)
...
**T5-16** 오써링 공수 정산 · `M` · 선행 T1-54, T3-18
  파일 `tools/calc_authoring.csx` (신규), `docs/measurements/authoring_result.md` (산출)
  사양 `docs/15 §7`
  내용 **두 축 모두 산출한다.**
...
**T5-17** 비용 실측 정산 · `S` · 선행 T4-15
  ...
  완료 상위계획 §10.2 추정표와 나란히 배치 · **오차율 제시**
...
```
W11 ── 골든 · 결정론 · 장애
[ ] T5-01 픽스처포맷+단언8종 [ ] T5-02 단언평가기      [ ] T5-03 픽스처50건 ★
