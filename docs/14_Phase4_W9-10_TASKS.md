...
```
[x] NPC 5,000 · 7게임일 완주                          10,080틱 완주 · 스텝 16,099,329 · 갭 0
[ ] 캐시 히트율 ≥ 98%                                 76.08% — 버킷 254/2,880 (전량 프리베이크 미실행)
[x] 틱 p99 ≤ 20ms                                     0.801ms  (P1 기준선 2.375ms)
[x] 인지 스캔 ≤ 150/틱 (5,000에서도, 10,000에서도)     150 / 150
[ ] GPU 사용률 ≤ 60%                                  미측정 — 전 회차 tier=none, models/ 비어 있음
[x] 시나리오 B 전 항목 통과                            SiegeTests 7건
[x] 일일 토큰 캡 초과 시 T2→T1 자동 강등               강등 1회 · T2 누계 캡 아래 유지
[x] T2 강제 실패 주입 시 T1 페일오버                   8건 전부 T1 처리 · 손실 0
[x] 틱 루프 Gen0 GC = 0                               0 (틱 창 할당 0 B)
[x] 큐 거절 발생 시에도 해당 NPC 정상 동작 (기존 플랜 유지)  거절 28,943건 · 플랜 없는 NPC 0/5,000
```

**8 통과 · 2 미달. `docs/14 §9` 는 전 항목 통과를 요구하므로 이 게이트는 아직 열리지 않았다.**
미달 둘은 코드가 아니라 **실측 산출물**이 없어서 판정되지 않은 것이고,
닫는 비용(전량 프리베이크 약 17.5분 · 약 $4.7 / 로컬 GGUF 배치 + T1 회차 재측정)은
`docs/measurements/P4_gate.md §4` 에 적혀 있다.
...
[x] T4-07 티어라우터 ★      [x] T4-08 스필오버         [x] T4-09 서킷브레이커
[x] T4-10 워커T1            [x] T4-11 워커T2×8         [x] T4-12 원자전달
[x] T4-13 버킷전환정식      [x] T4-14 LOD갱신정식

W10 ── 부하 · 튜닝 · 대시보드 · 시나리오
[x] T4-15 부하하네스        [x] T4-16 스케일곡선 ★     [x] T4-17 가중치A/B ★
[x] T4-18 캐시패널          [x] T4-19 재계획패널       [x] T4-20 비용패널
[x] T4-21 NPC추적패널 ★     [x] T4-22 버킷히트맵 ★
[x] T4-23 시나리오B ★       [x] T4-24 게이트검증 ★
```
