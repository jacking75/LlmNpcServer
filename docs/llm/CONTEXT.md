...
`npc explain core` 는 빌드에 고정된 플래그·POI 역할·버킷 축·기본 액션을 보여 준다.
다른 세계의 `world_flags.json` 은 빌드에 쓰인 이름·bit 와 같아야 한다.
...
| 항목 | 값 |
|---|---|
| 틱 p99 (NPC 5,000) | **0.291 ms** (2026-09-24 재측정, 예산 20ms) · 틱당 할당 **0 B** |
