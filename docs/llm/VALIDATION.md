...
| `V12` | duty_hours 없이 Guard·Patrol 을 허용했다. duty_hours 를 주거나 그 액션을 뺀다 — OnDuty 를 세우는 것은 duty_hours 뿐이라, 없으면 인지 스캔이 매 틱 이탈로 읽어 재계획 큐가 포화한다. | `CLAUDE.md#7` · `docs/reference_masterdata.html#v12` |
| `V13` | npc_instances.json 의 참조·정원·순찰로가 어긋났다. 손편집은 npc_overrides.json 에 하고 `npc regen` 으로 생성물을 다시 만든다. | `docs/reference_masterdata.html#v13` |
