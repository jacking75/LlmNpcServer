...
2. **핸드셰이크를 맞춘다.**
   - `npc export link-bundle --npcs <N> --out bundle.json`으로 만든 번들에서 구조·내용·로스터 해시와 스폰할 NPC의 순서를 읽는다. `--zone`·`--shard`는 NPC 서버와 같게 준다.
   - 프로토콜 버전 범위 [1, 2] 협상.
   - **구조 해시는 완전 일치**를 요구한다. 불일치면 거절이다.
   - 내용 해시 불일치는 경고 후 수락 — 다른 파이프라인으로 배포되는 것이 정상이다.
   - 인증을 켰으면 HMAC-SHA256 + 논스.
3. **이벤트 발행 지점을 정한다.** `docs/reference_link.html` §05의 "명령별 응답 규약" 표를 명령 핸들러 옆에 놓고 각 성공·실패 이벤트를 매핑한다. C6 적합성 검사가 응답 종류와 상관 ID를 확인한다.
