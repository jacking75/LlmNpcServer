...
[~] T0-09  강제 디코딩 100회 러너        ★ 게이트 G0-1  — 산출물 커밋됨 · 게이트 미달 (§아래 처리)
[~] T0-10  성능 측정 하네스              ★ G0-2, G0-3   — 산출물 커밋됨 · G0-2 미달 · 6조합(로컬 2종)
[~] T0-11  동시성 측정                                  — 로컬만. 외부 API 미측정
[~] T0-12  품질 수동 평가                ★ 게이트 G0-4  — 4B만 채점. 8B 없어 판정 불가
[x] T0-13  결과 정리 및 상위 문서 갱신                  — W1_results.md · 상위계획 §2.1/§10 · CLAUDE.md §9 · docs/03 §2
```

**`[~]` 는 "산출물은 나왔는데 완료 조건을 못 채웠다"는 뜻이다.** 넷 다 커밋은 끝났고(2026-07-28 확인), 남은 것은 **측정 범위**다 — 외부 API 를 한 번도 부르지 않았고 8B 를 채점하지 않았다.

**T0-13 은 `x` 다.** 미달·미측정을 그대로 적는 것이 T0-13 의 완료 조건이었고, 그렇게 적었다. **넷을 `x` 로 올리려면 새 측정 회차가 필요하다** — `W1_results.md §5` 에 무엇이 필요한지 있다.

---

## 게이트 판정 (2026-07-26 기준)

| 게이트 | 기준 | 실측 | 판정 |
|---|---|---|---|
| **G0-1** | 어휘 검증 통과율 최고 엔진 ≥ 90/100 (2026-07-28 개정) | `gemini-3.1-flash-lite` **93/100** (prompt 모드) | **통과** |
| **G0-2** | 요청당 지연 1.75s ±50% (0.875~2.625s) | 로컬 4B **3.7s** · 8B **5.1s** (캐시 적중, 중앙값) | **미달** |
| **G0-3** | 캐시 적중 시 prefill 70% 이상 감소 | 4B 1,531 → 237ms (**85%**) · 8B 2,608 → 316ms (**88%**) | **통과** |
| **G0-4** | 4B가 8B의 80% 이상 | 4B 7.54/10 만 채점. **8B 미채점** | **판정 불가** |

측정 조건은 `docs/measurements/W1_env.md` 를 반드시 같이 본다 — **VRAM 8GB** 가 이 수치들의 가장 큰 제약이다.

### 미달을 어떻게 처리했는가

- **G0-1** — 단순 미달이 아니라 **위험 신호 표가 못 잡는 실패였다.** `docs/10 §5` 의 신호는 "강제 디코딩이 90% 미만"인데 forced 는 유효 JSON 99/100 으로 그 선을 넘었다. 문제는 그 다음이다 — **어휘 검증이 0/100** 이었다(`V2.UNKNOWN_ARG` 99). 문법을 지키면서 인자를 지어내니 파서는 통과시킨다. 같은 모델의 prompt 모드는 93/100 통과였다. 그래서 스키마를 더 조이는 대신 **강제 디코딩을 기본에서 뺀다.** `docs/12 §4`에 근거를 적었고 T2-02·T2-09가 이걸 따른다.
  **2026-07-28 — 기준 자체를 "어휘 검증 통과율 ≥ 90/100" 으로 바꿨고, 그 기준으로는 93/100 통과다.** 측정 지표를 잘못 고르면 100% 실패를 통과로 읽는다는 것이 이 게이트에서 배운 것이라, 배운 것을 기준에 반영했다.
- **G0-2** — 지연이 가정의 2~3배다. `../TASKS.md §5` 표의 "지연이 가정의 2배 이상" 행이 발동했다. P4 예산(`docs/14 §3`)을 실측 기준으로 고쳤고, 상위 계획 §2.1/§10 과 `../CLAUDE.md §9` 갱신은 **T0-13에서 끝냈다** (2026-07-28). 원인은 prefill 이 아니라 **decode** 다 — 가정 100 tok/s 에 실측 44 tok/s(8B)다. prefill 은 캐시가 걸리면 가정한 0.25s 에 붙는다(`measurements/W1_results.md` M2).
- **G0-4** — 8B 품질 채점이 없어 판정할 수 없다. 8B 를 같은 조건으로 돌리거나(권장), 못 하면 "로컬 모델 확정"을 **미결정으로 두고** P4 티어 설정을 4B 기준으로 잡는다.

**원래 규약은 게이트 4개를 전부 통과해야 P1로 넘어가는 것이었으나, P1(T1-01~T1-62)은 이미 완료됐다.**
다행히 P1은 LLM 을 한 번도 부르지 않는 범위라 위 미달 항목의 영향을 받지 않는다(`P1_gate.md` #2: LLM 호출 0).
**영향은 전부 P2 이후에 있고**, 그래서 T0-13 을 P2 착수 전에 끝내야 한다 — `docs/12`~`docs/15` 의 태스크는 이미 위 실측을 반영해 고쳐 뒀다.
