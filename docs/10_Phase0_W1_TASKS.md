...
[~] T0-09  강제 디코딩 100회 러너        ★ 게이트 G0-1  — 산출물 있음 · 게이트 미달 · 미커밋
[~] T0-10  성능 측정 하네스              ★ G0-2, G0-3   — 산출물 있음 · G0-2 미달 · 로컬만 · 미커밋
[~] T0-11  동시성 측정                                  — 로컬만. 외부 API 미측정 · 미커밋
[~] T0-12  품질 수동 평가                ★ 게이트 G0-4  — 4B만 채점. 8B 없어 판정 불가 · 미커밋
[ ] T0-13  결과 정리 및 상위 문서 갱신                  — W1_results.md 없음
```

**`[~]` 는 "산출물은 나왔는데 완료 조건을 못 채웠다"는 뜻이다.** 규약상 `x` 는 커밋까지 끝난 것이고, 위 넷은 아직 워킹 트리에만 있다.
...
- **G0-2** — 지연이 가정의 2~3배다. `../TASKS.md §5` 표의 "지연이 가정의 2배 이상" 행이 발동했다. P4 예산(`docs/14 §3`)을 실측 기준으로 고쳤다. 상위 계획 §2.1/§10 과 `../CLAUDE.md §9` 갱신은 **T0-13에 남아 있다.**
