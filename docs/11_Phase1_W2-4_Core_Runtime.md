...
슬라이스만으로는 상한이 지켜지지 않는다. LOD 0 은 주기가 1이라 밴드 전원을 매 틱 본다 — 플레이어가 광장에 몰리면 수백 마리가 한꺼번에 LOD 0으로 올라오고 스캔이 그만큼 늘어난다. 실측으로 NPC 5,000·플레이어 봇 20에서 틱당 612건이 나왔다.

그래서 `CognitionScheduler.MaxScansPerTick = 150`으로 틱당 총량을 자르고, **밴드별 커서**를 돌려 잘린 뒤쪽을 다음 틱에 이어 본다. 군중이 생기면 그 밴드의 판정 주기가 늘어날 뿐이고, 굶는 NPC는 없다. 스캔 비용이 인구와 무관해지는 것이 이 구조의 목적이다.
...
### 틱 경계 규칙 — 결정론의 전제 (T5-07 에서 추가)

위 스케치는 "이벤트가 오면 배수하고 시계가 진행되면 한 틱을 돈다"까지만 정한다. 그것만으로는 **같은 시나리오를 두 번 돌려도 결과가 다르다** — 실측으로 이벤트 30,416 vs 30,649 였다. 배수량과 flush 시점이 스레드 스케줄에 달려 있기 때문이다. 세 가지를 못 박는다.

| # | 규칙 | 안 지키면 |
|---|---|---|
| 1 | **이벤트 배수는 `TickSync` 를 만나면 끊는다.** 게임서버(대역)는 틱마다 `TickSync` 를 그 틱의 **마지막** 이벤트로 낸다 — 그 경계가 곧 틱 경계다 | 생산자가 앞서 나간 만큼 다음 틱의 이벤트가 한 배수에 딸려 들어와, **아직 돌리지 않은 틱의 결과가 이번 틱의 판단에 반영된다** |
| 2 | **`FlushAsync` 는 틱마다 한다.** 바깥에서 한 번만 하지 않는다 | 이번 틱의 명령이 링크 큐에 남은 채로 틱 카운터가 먼저 오른다 |
| 3 | **게임서버 대역의 페이싱 기준은 `TicksCommitted`(flush 까지 끝난 틱)이고, 기다리는 자리는 세계를 밀기 <b>전</b>이다** | 밀고 나서 기다리면 다음 틱의 이벤트가 이미 채널에 들어간 뒤다. `TicksProcessed` 로 재면 명령이 아직 큐에 있는 순간에 세계가 넘어간다 |

세 규칙이 다 서면 `--link record` 두 번이 바이트 동일이 되고, `--link replay` 도 같은 명령 열을 낸다(`docs/15 §3`). 대역이 대기하는 자리에서는 `SpinWait.SpinOnce(sleep1Threshold: -1)` 을 쓴다 — 락스텝이라 매 틱 한 번은 기다리게 되는데 거기서 `Thread.Sleep(1)` 로 넘어가면 1,440틱짜리 하루가 그것만으로 20초를 더 쓴다.
