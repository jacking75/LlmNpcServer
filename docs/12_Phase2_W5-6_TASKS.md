...
**판정: 7개 중 3개 통과 — 미달.** 근거는 `docs/measurements/W6_compile_stats.md §8` 이다.
통과율은 고른 표본 60버킷(78.3%)과 연속 288버킷(64.6%, 제작 계열) 둘 다 있고,
폴백·다양성은 인접 재사용이 동작하는 **연속 288버킷** 값이다. 전량 2,880 회차는 안 돌렸다.

```
[ ] 2,880 버킷 전량 생성 시 검증 통과율 ≥ 90% (재시도 1회 포함)   78.3% (표본 60) · 64.6% (연속 288)
[ ] 폴백으로 떨어지는 비율 ≤ 5%                                   15.6% (연속 288. 재사용이 실패 102건 중 57건을 건졌다)
[x] 서픽스 토큰 ≤ 300 (전 버킷·스트레스 케이스·재시도 케이스)     최악 300
[x] 프리픽스 SHA가 전 요청에서 1종                                1종
[ ] 프롬프트 캐시 적중률 ≥ 95% (외부 API — cached_tokens 기준)    41.7% (288건 회차. 회차가 길수록 오른다)
[x] 드라이런 결정론 (같은 플랜 100회 → 동일 판정)                 동일
[ ] 플랜 다양성 ≥ 60%                                             37.1% (연속 288버킷)
```
...
W6 ── 검증기완성 · 실패처리 · 개선루프
[x] T2-12 Explain보강       [x] T2-13 DryRunValidator  [x] T2-14 드라이런결정론
[x] T2-15 재시도(1회)       [x] T2-16 인접버킷재사용   [x] T2-17 폴백경로
[x] T2-18 rejected보존
[x] T2-19 전량생성러너      [x] T2-20 실패집계리포트   [x] T2-21 개선루프 ★
[x] T2-22 다양성지표 ★      [-] T2-23 count심볼화(조건) [x] T2-24 W6리포트
```
