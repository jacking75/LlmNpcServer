...
**판정: 7개 중 4개 통과 — 미달.** 근거는 `docs/measurements/W6_compile_stats.md §8` 이다.
통과율·폴백은 60버킷 표본으로 잰 값이고, 전량 2,880 회차는 아직 안 돌렸다.

```
[ ] 2,880 버킷 전량 생성 시 검증 통과율 ≥ 90% (재시도 1회 포함)   실측 78.3% (표본 60)
[ ] 폴백으로 떨어지는 비율 ≤ 5%                                   실측 21.7% (표본이라 인접 재사용이 0)
[x] 서픽스 토큰 ≤ 300 (전 버킷·스트레스 케이스·재시도 케이스)     최악 300
[x] 프리픽스 SHA가 전 요청에서 1종                                1종
[ ] 프롬프트 캐시 적중률 ≥ 95% (외부 API — cached_tokens 기준)    실측 14.4% (OpenRouter 암묵 캐시)
...
[x] T2-18 rejected보존
[~] T2-19 전량생성러너      [x] T2-20 실패집계리포트   [x] T2-21 개선루프 ★
