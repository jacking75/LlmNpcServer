...
**7 통과 · 1 부분 · 2 미판정 (2026-08-01).** 부분은 G6-5 한 항목이고 **계기 선택의 문제**다
(실측은 예산의 100배 여유다). 미판정 둘은 **사람이 눈으로 보는 항목**이다 —
...
- [~] G6-5 틱 예산 유지 — **10분 회차를 돌렸다 (2026-08-01).** `--link tcp --npcs 500 --time-scale 60`, 예열 60초 뒤 **6,002틱** 측정. **p99 0.197ms · p50 0.041ms · max 1.073ms · 오버런 0** (예산 20ms — 100배 여유). **`bytesPerTick` = 0** · 명령 221,610건 전량 flush · 드롭 0 · 갭 0. 게임서버 쪽 p99 0.65~0.70ms(예산 10ms). **판정을 `[x]` 로 못 올린 것은 "Gen0 증가 0" 한 항목 때문이다** — 아래 참조
...
> **G6-5 의 "Gen0 증가 0" 을 무엇으로 재는가 — 결정 대기 (2026-08-01).**
...
>
> 셋 중 하나를 골라야 한다.
> (1) 판정을 `bytesPerTick == 0` 으로 바꾼다 — 재려던 것을 직접 재는 계기다.
> (2) "정상 상태 Gen0 증가 0" 으로 바꾼다 — 결정 17 이 `Gate_NoAllocationInTickLoop` 에 쓴 처방과 같은 모양.
> (3) 회차에서 `--no-dashboard` 를 주고 `/metrics` 대신 종료 요약으로 재서 전역 계기를 깨끗하게 만든다.
>
> **임의로 고르지 않았다.** 결정 17 이 같은 종류의 판정 변경을 사람의 결정으로 남긴 전례다.
