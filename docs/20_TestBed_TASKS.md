...
| G6-5 | 틱 예산 유지 | `--link tcp --npcs 500`으로 10분 → p99 ≤ 20ms · 오버런 0 · **`bytesPerTick` == 0** (2026-08-01 결정 1-A. 원래 "Gen0 증가 0"이었는데 그 계기가 프로세스 전역이라 틱 루프를 재지 못한다 — §4 참조) |
...
**9 통과 · 1 미판정 (2026-08-01).** 남은 하나는 G6-10 — **사람이 화면 앞에 앉는 일**이고
이 Phase 의 목적이다. 다만 G6-9 회차에서 **그 사람이 볼 것 하나가 관측되지 않았다**(아래
`planBucket` 항목) — G6-10 을 판정하기 전에 그것부터 갈라야 한다.
...
- [x] G6-5 틱 예산 유지 — **10분 회차 통과 (2026-08-01).** `--link tcp --npcs 500 --time-scale 60`, 예열 60초 뒤 **6,002틱** 측정. **p99 0.197ms · p50 0.041ms · max 1.073ms · 오버런 0** (예산 20ms — 100배 여유). **`bytesPerTick` = 0** · 명령 221,610건 전량 flush · 드롭 0 · 갭 0. 게임서버 쪽 p99 0.65~0.70ms(예산 10ms). 판정 계기를 `gen0Collections`(프로세스 전역)에서 `bytesPerTick`(틱 루프 전용)으로 바꿨다 — 아래 참조
...
> 그것이 이 항목이 지키려던 것이다 (`CLAUDE.md` §2.1 "Gen0 GC = 0 이 목표"는 **틱 루프**의 이야기다).
>
> 그런데 같은 회차에서 `gen0Collections` 는 1 → 5 로 **4 늘었다.** 이 값은
> `NpcMeter.Gen0Collections` = `GC.CollectionCount(0) - _gen0AtStart` 로 **프로세스 전역**이다 —
> 틱 루프 밖의 ASP.NET 대시보드(측정하려고 `/metrics` 를 세 번 긁었다)·TCP 리시버/센더·
> MemoryPack 직렬화가 전부 여기 든다. **틱 루프를 재는 계기가 아니다.**
>
> **판정을 계기에 맞춘 것이지 실측에 맞춘 것이 아니다.** 그 둘은 다르다 — 실측에 맞추는 것이라면
> "Gen0 증가 ≤ 5" 같은 하한을 새로 만들었을 것이고, 그러면 진짜 누수가 생겨도 안 걸린다.
> `bytesPerTick` 은 **틱 루프가 할당한 바이트를 직접 세므로** 하한이 0 그대로다.
>
> 나머지 둘은 안 골랐다. "정상 상태 Gen0 증가 0"(결정 17 의 처방)은 여전히 전역 계기를 쓰는 것이라
> 대시보드를 열어 두면 또 흔들리고, `--no-dashboard` 로 재는 것은 **측정하려고 관측을 끄는** 모양이 된다.
