...
| 첫 실행 데모 플랜 | 생성 242건 + 고정 19건, 100 NPC 회차 1,312틱 시점 버킷 캐시 적중률 98.14% |
| 상황 플랜 생성 비용 | 2,880건 과거 회차 $5.12, 이번 데모 팩 $0.347 (워밍업 포함) |

측정 조건과 미측정 항목은 [성능·한계 레퍼런스](docs/reference_metrics.html)에 있다. 데모 팩의 평시 도달 집합 264개 중 17개는 생성 검증에 계속 실패해 기본 행동으로 해소된다. 비용은 해당 회차의 값이며 다른 세계에 그대로 적용되지 않는다.
...
저장소 코드는 [MIT 라이선스](LICENSE)로 배포한다. dotLLM은 GPLv3 별도 프로그램이므로 이 저장소와 배포 zip에 포함하지 않는다. 모델을 연결할 때는 [모델별 약관](docs/legal/models.md)을 확인한다.
