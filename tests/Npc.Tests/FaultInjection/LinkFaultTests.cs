...
public sealed class LinkFaultTests
{
    ...
    {
        long clock = 0;
        var breaker = new CircuitBreaker(() => clock, threshold: CircuitBreaker.FailureThreshold, openMillis: 60_000);

        var t2 = new TimingOutCompiler();
        var t1 = new AlwaysOkCompiler();
        var router = new TieredPlanCompiler(t2, t1, switches: null, breaker: breaker);

        // 30초 타임아웃이 연달아 난다. 임계(5회)에서 열려야 한다.
        for (int i = 0; i < CircuitBreaker.FailureThreshold; i++)
        {
        ...
        Assert.Equal(CircuitBreaker.FailureThreshold, t2.Calls);
