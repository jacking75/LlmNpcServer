...
>
> **후속 지시서는 [`STUDIO_IMPROVEMENT_PLAN_2.md`](STUDIO_IMPROVEMENT_PLAN_2.md) 다** (2026-09-17 · 보강 28건 · **완료**).
> 이 문서가 ✅ 로 표시했지만 코드에 없는 것 일곱 가지(T10 떠날 때 확인 · T12 ④ 편집기 · T13 지도 클릭 ·
> T15 IssuePanel/`?field=` · T17 확인 모달 · T27 탭 숨김 · T32 Ctrl+K)는 그쪽 §1.3 에 표로 있고 거기서 끝냈다.
...
| 11 | [x] | **T24** | **동작 미리보기 화면** — SVG NPC 인형 · 지도 재생 · 시간 스크러버 · 상황 버튼 | F 예측 | P0 | L | T07 T22 T23 |<br>→ 2차 **H20**(반응 이동 애니메이션) · **H24**(키보드 · 스크러버 드래그)에서 마무리
...
| 22 | [x] | **T26** | 마을 전체 지도 — 지역 타일 12개 · 인구 밀도 · 클릭 이동 | F 예측 | P1 | M | T07 T08 |<br>→ 2차 §1.3: `ZoneLayout.Compute` 를 `Narrative` 로 옮기지 않는다. `StudioPlaces.Tiles()` 가 결정론을 지킨다
...
| 28 | [x] | **T32** | 전역 검색 — 직업·NPC·장소를 한국어/ID 로 한 칸에서 | B 읽기 | P1 | S | T01 T02 |<br>→ 2차 **H19**: `Ctrl+K` 는 문구뿐이었다. 거기서 실제로 이었다
| 29 | [x] | **T27** | 버킷별 계획 — 미리 구운 플랜(`planstore`) 을 상황 격자로 보고 예측한다 | F 예측 | P1 | M | T22 T24 |<br>→ 2차 **H20**: `--planstore` 가 없을 때 탭을 숨기는 것이 빠져 있었다
...
5. **결정(2026-09-16).** 파생물 재생성은 **연습장에서 막는다.** 생성기에 입력 디렉터리 인자가 없다 — `gen_npcs.cs` 는 `--out` 만 받고 `gen_poi_distances.cs` 는 인자가 아예 없으며, 둘 다 저장소의 `masterdata/` 를 읽는다. 연습장에 대고 돌리면 **원본을 읽어 연습장에 쓴다** — 연습장에서 고친 `pois.json` 이 반영되지 않은 거리표가 나오고 그것은 조용히 틀린 파일이다. `GeneratorRunner.CanRunFor` 가 경로를 비교해 막고, 화면이 "원본에 적용한 뒤 만든다" 를 적는다.
...
| ④ 하루 일과 | 원본 폴백 초안 → T11 편집기. 원본 레시피·아이템 노란 표시 · **오른쪽에 예측 재생**(T24) | V8 · ✓ |<br>→ **실제로는 안내문 두 줄이었다.** 2차 **H06** 이 편집기를 넣었다
