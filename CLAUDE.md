...
> **주차별 작업 지시서(`docs/1x_*_TASKS.md` · `docs/20_TestBed_TASKS.md`)는 구현 완료로 삭제했다 (2026-08-06).**
> 개별 태스크의 파일 목록·사양 참조·완료 조건이 필요하면 `git log --diff-filter=D -- docs/` 에서 꺼낸다.
> 남아 있는 것은 **규약과 진행 원장(`TASKS.md`)** 과 **설계 사양(`docs/1x_Phase*.md`)** 이고, 이 둘이면 근거는 따라갈 수 있다.
...
3. 선행 태스크가 TASKS.md §3 원장에서 완료로 잡혀 있는지 확인한다
4. "파일" 에 적힌 경로에만 만든다
...
testbed/Npc.TestGameServer    ←  Sim, MasterData, Wire, Protocol   [T6-23 완료]
