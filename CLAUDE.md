...
| 프리픽스 토큰 수 **4,200~4,500** (하한 4,096) | Gemini 3.x / Claude Haiku 4.5 캐시 최소 임계 |
...
| 항목 | 값 | 출처 |
|---|---|---|
| 로컬 요청당 지연 | ~1.75s (8B Q4, 프리픽스 캐시 적중) | W1 실측으로 갱신 |
| 로컬 처리량 | ~2,050 req/h (continuous batching 없음) | 상위 계획 §2.1 |
| 외부 API 처리량 | ~57,600 req/h (동시 32) | 상위 계획 §10.1 |
| 버킷 수 | 40 × 6 × 4 × 3 = **2,880** | `docs/01` §6 |
| 프리베이크 | 외부 동시 32로 ~3분, ~$0.3 | 상위 계획 §10.2 |
| dotLLM 특성 | decode는 llama.cpp의 0.8x, **prefill은 2~5배 느림** | 프리픽스 캐시가 필수인 이유 |

W1 스파이크 이후 실측치가 나오면 `docs/measurements/W1_results.md`에 기록하고 이 표와 상위 계획서를 갱신한다.
