...
public enum PlanOrigin : byte
{
    ...
    /// <summary>
    /// 결정론 수선을 거쳐 통과한 플랜 (C-05).
    ///
    /// <para>
    /// <b>검증을 건너뛴 것이 아니다.</b> 수선 뒤 4단 검증을 처음부터 다시 통과했다 —
    /// 그래도 따로 표시하는 이유는 <b>검수 표본에서 가려 보기 위해서</b>다. "기계가 고친 것" 이
    /// 모델이 낸 것과 섞이면, 수선 규칙이 나쁜 플랜을 통과시키고 있어도 알아챌 계기가 없다.
    /// </para>
    ///
    /// <para><b>번호는 뒤에만 붙인다</b> — <c>code</c>·<c>bit</c> 와 같은 규칙이다 (CLAUDE.md §2.4).</para>
    /// </summary>
    Repaired = 4,
}
