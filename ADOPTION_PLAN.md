...
- [x] [T1. 라이선스를 정하고 명시한다](#t1) — 완료 2026-09-24 14:12 KST (MIT 원문·README·NuGet 메타데이터 일치)
...
- [x] [T6. 파이썬 최소 게임서버 예제 — 적합성 키트 통과까지](#t6) — 완료 2026-09-24 08:49 KST; 13:58 KST `--probe` 실송신 3/3 응답으로 C6 재확인
...
- [x] [T10. 데모 플랜 팩](#t10) — 완료 2026-09-24 14:12 KST (생성 242·고정 19, 격리 회차 캐시 적중 98.14%, 깨끗한 클론에서 키 없이 정상 종료, 워밍업 포함 $0.347332)
...
- [ ] [T14. HTML 문서를 GitHub 에서 읽히게 한다 (GitHub Pages)](#t14) — 사용자 결정: 공개하지 않음. Pages 활성화·URL 검증 제외
...
- [x] [T19. 배포 산출물(자체 포함 zip) + 플랫폼 표기 정정](#t19) — 완료 2026-09-24 14:14 KST (Windows·WSL Linux SDK 없이 500 NPC 하루 정상, 3종 zip·SHA256SUMS를 v1.0.0 Release에 게시; macOS 실행 미검증)
...
| #       | 결정                | 선택지                                                                | 권장                                                             |
| ------- | ----------------- | ------------------------------------------------------------------ | -------------------------------------------------------------- |
| T1      | 저장소 라이선스          | Apache-2.0 · MIT · 비공개 전환                                          | **MIT 확정** — 사용자 결정 |
| T2 · T4 | 계약 **의미** 변경 승인   | ① `timeout_s` 를 "하한"으로, 마감은 예상 소요에서 도출 ② 즉시형 명령의 지속 시간은 NPC 서버가 센다 | 둘 다 승인 — 와이어·패킷은 안 바뀌고 레퍼런스 문서의 문장만 바뀐다                        |
| T10     | 데모 플랜 팩           | A 커밋 · B 릴리스 첨부 · C 안 함                                            | **A 확정** — 약 $0.37 비용 승인 |
| T14     | GitHub Pages 켜기   | 켠다 · 안 켠다                                                          | **안 켠다 확정** — 사용자 최신 결정 |
| T19     | GitHub Release 게시 | 게시 · zip 만 만든다                                                     | **게시 확정** — 사용자 결정 |
...
3. `dotnet run -c Release --project tools/Npc.Prebake -- --masterdata ./masterdata --out ./planstore --tier T2 --model openrouter-gemini-2.5-flash-lite --concurrency 8 --budget-usd 0.40 --buckets-file reachable-buckets.txt` (`--resume`으로 실패한 버킷만 재시도)
...
**결정** 사용자 최신 답변에 따라 GitHub Pages를 공개하지 않는다. 아래 활성화 절차와 공개 URL 검증은 수행하지 않는다.
